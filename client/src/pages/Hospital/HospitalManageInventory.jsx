...
import { Building2, CalendarDays, AlertTriangle, Stethoscope, Droplet } from "lucide-react";

const bloodGroups = ["A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-"];
const components = ["Whole Blood", "RBC", "Plasma", "Platelets"];
const sources = ["Donation", "Purchase", "Transfer", "Camp"];

function HospitalManageInventory() {
  const { user } = useAuth();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white via-gray-50 to-blue-50">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-gray-800 flex items-center gap-2"><Building2 className="text-blue-600" /> Hospital Inventory</h2>
          <p className="text-sm text-gray-500 mt-1">Capture inbound units and monitor freshness. Role: <span className="font-medium text-gray-700">{user?.role || "—"}</span></p>
        </div>
        <div className="flex gap-2 text-xs">
          <div className="px-3 py-1 rounded-full bg-blue-100 text-blue-700">Inbound Tracking</div>
          <div className="px-3 py-1 rounded-full bg-cyan-100 text-cyan-700">Auto Expiry Logic</div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="mt-6 bg-white/90 backdrop-blur rounded-2xl shadow-sm border border-blue-200 p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
        <div>
          ...
          <select name="sourceType" value={form.sourceType} onChange={handleChange} className="w-full border border-gray-300 focus:ring-blue-500 focus:border-blue-500 rounded-lg px-3 py-2 text-sm" required>
        ...
        <div>
          ...
          <input name="receivedDate" type="date" value={form.receivedDate} onChange={handleChange} className="w-full border border-gray-300 focus:ring-blue-500 focus:border-blue-500 rounded-lg px-3 py-2 text-sm" required />
        </div>
        <div>
          <label className="block text-xs font-semibold tracking-wide text-gray-600 mb-1 flex items-center gap-1"><AlertTriangle className="w-3 h-3" /> Expiry Date</label>
          <input name="expiryDate" type="date" value={form.expiryDate} onChange={handleChange} className="w-full border border-gray-300 focus:ring-blue-500 focus:border-blue-500 rounded-lg px-3 py-2 text-sm" />
        </div>
        ...
        <div className="overflow-x-auto rounded-xl border border-gray-200 shadow-sm bg-white">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              ...
              {entries.length === 0 && (
                <tr>
                  <td className="p-6 text-center text-gray-500" colSpan={6}>No entries yet</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <div className="mt-12">
        <h3 className="text-lg font-semibold text-gray-800 mb-3 flex items-center gap-2"><AlertTriangle className="w-5 h-5 text-blue-600" /> Expiry Alerts</h3>
        <div className="mt-3">
...
export default HospitalManageInventory;
