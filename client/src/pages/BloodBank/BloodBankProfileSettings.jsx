...
import { Package2, Mail, Phone, MapPin, ShieldCheck } from "lucide-react";
import axios from "axios";
import Navbar from "../../components/BloodBankComponent/Navbar";
import { useNavigate } from "react-router-dom";

export default function BloodBankProfileSettings() {
  ...
  const bank = userData?.bloodBankDetails || {};
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto">
        <h2 className="text-2xl font-bold text-gray-800 mb-1">Blood Bank Profile</h2>
        <p className="text-gray-500 mb-8">Manage your blood bank information and settings.</p>
        <div className="grid md:grid-cols-3 gap-8">
          <div className="bg-white rounded-2xl shadow-sm p-6">
            <div className="flex flex-col items-center">
              <Package2 className="w-12 h-12 text-red-500 mb-4" />
              <h3 className="font-semibold text-lg text-gray-800">{userData.fullName}</h3>
              <p className="text-gray-500 text-sm mb-2">{bank.email || userData.email}</p>
              {bank.licenseNumber && (
            ...
            <div className="mt-6 border-t pt-4 text-sm space-y-3">
              <div className="flex justify-between text-gray-600">
                ...
                <span>{bank.hasBloodBank ? "Available" : "Not Available"}</span>
              </div>
              <div className="flex justify-between text-gray-600">
                <span className="flex items-center gap-2">
                  <MapPin className="w-4 h-4 text-blue-500" /> City
                </span>
                <span>{bank.city || userData.city || "N/A"}</span>
              </div>
            </div>
          </div>
          <div className="md:col-span-2 space-y-6">
            <form onSubmit={handleSave} className="bg-white rounded-2xl shadow-sm p-6">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-sm text-gray-600">Blood Bank Name</label>
                  <input
                    type="text"
                    name="fullName"
                ...
                <div>
                  ...
                  <input
                    type="email"
                    name="email"
                    value={bank.email || userData.email || ""}
                    onChange={handleBankChange}
                    disabled={!isEditable}
                    className={`w-full mt-1 p-2 border rounded-lg ${!isEditable ? "bg-gray-100 cursor-not-allowed" : ""}`}
                  />
                ...
                <div>
                  ...
                  <input
                    type="text"
                    name="phone"
                    value={bank.phone || userData.phone || ""}
                    onChange={handleBankChange}
                    disabled={!isEditable}
                    className={`w-full mt-1 p-2 border rounded-lg ${!isEditable ? "bg-gray-100 cursor-not-allowed" : ""}`}
                  />
